function [data,events,hdr] = Copy_of_Data_cut_window_online(traindata,traindevents,hdr,tot_trial,train_wndow,init_offset,ovrl,ovrl_wndow)
%
...
for i=1:num_wind   %loop to concatenate several train_wndow within same 5s trial,to obtain [ch x time x epoch] data
    ...
    smp_offset = (offs/1000)*hdr.Fs; %hdr.fsample;
    smp_train_wndow = (train_wndow/1000)*hdr.Fs;%fsample;
    
    for m = 1:length(traindevents) %number of events/epochs     
        for h = 1:hdr.nChans%nchans %number of channels         
