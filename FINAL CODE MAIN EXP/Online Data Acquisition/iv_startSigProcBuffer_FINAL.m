...
while ( true )
    ...
    switch lower(phaseToRun);
        ...
        case {'calibrate','calibration'};   % % INES'S EXPERIMENT
            ...
            fname=[dname '_' subject '_sub' cfgcls.sub '_session' cfgcls.session]; %training data name
            fprintf('Saving %d epochs to : %s\n',numel(traindevents),fname);
            save([cfgcls.pth_lab3 '/' fname '.mat'],'traindata','traindevents','hdr'); %save in subj specific folder in output
            trainSubj=subject;
            ...
            save([cfgcls.pth_lab3 '/' cutdata_name],'traindata','traindevents','hdr'); %cut and save data in smaller windows to train classifier
        ...
        case {'train','training','trainerp','trainersp','train_subset','trainerp_subset','trainersp_subset','train_useropts','trainerp_useropts','trainersp_useropts'};
            ...
            if ( ~isequal(trainSubj,subject) || ~exist('traindata','var') )
                ...
                % % INES'S EXPERIMENT
                fname=[dname '_' subject '_sub' cfgcls.sub '_session' cfgcls.session]; %training data name
                fprintf('Loading cut training data from : %s\n',fname);
                cutdata_name = strcat(fname,'_cut.mat');
...
%                 end
            ...
            switch lower(clsfr_type);
                ...
                case {'ersp','induced'};
                    ...
                    %% INES'S EXPERIMENT
                    %automatically save train plots to subject specific folder in output
                    plotdir = sprintf([cfgcls.pth_lab3 '/plots']);
                    mkdir(plotdir);
                    saveas(2, fullfile(plotdir, sprintf('ersp_sub%s_session%s',cfgcls.sub,cfgcls.session)), 'png');
                    saveas(3, fullfile(plotdir, sprintf('auc_sub%s_session%s',cfgcls.sub,cfgcls.session)), 'png');
                    saveas(4, fullfile(plotdir, sprintf('confusion_sub%s_session%s',cfgcls.sub,cfgcls.session)), 'png');
                    
                otherwise;
                    error('Unrecognised classifer type');
            end
            clsSubj=subject;
            fname = ['clsfr_' subject '_sub' cfgcls.sub '_session' cfgcls.session]; %classifier name
            fprintf('Saving classifier to : %s\n',fname);
            save([cfgcls.pth_lab3 '/' fname '.mat'],'-struct','clsfr');
            resname = ['res_' subject '_sub' cfgcls.sub '_session' cfgcls.session]; %training results name
            fprintf('Saving results to : %s\n',resname);
            save([cfgcls.pth_lab3 '/' resname '.mat'],'-struct','res');
